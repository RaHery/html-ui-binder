...
public class HtmlUiBinderGenerator extends Generator {
    ...
    public String generate(TreeLogger logger, GeneratorContext context, String typeName)
        ...
        if (sourceWriter != null) {
            logger.log(Type.INFO, "Generating HtmlBinder for type `" + typeName + "'" + " Context=" + context);
            Document htmlDocument = readDocument(logger, context.getResourcesOracle(), requestedType);

            PartBuilder[] generators = new PartBuilder[] { new SafeHtmlTemplateBuilder(), new WidgetFieldsBuilder(),
                    new EventHandlerBuilder(), new CreateAndBindUiBuilder(), };

            // Do generation
            HtmlUiGeneratorContext ctx = new HtmlUiGeneratorContext(htmlDocument, context, requestedType, logger,
                    sourceWriter);
    ...
    private SourceWriter getSourceWriter(TreeLogger logger, GeneratorContext ctx, JClassType requestedType) {
        ...
        ClassSourceFileComposerFactory composerFactory = new ClassSourceFileComposerFactory(packageName,
                getBinderSimpleName(requestedType));
    ...
    private String deduceTemplateFile(TreeLogger logger, JClassType interfaceType) throws UnableToCompleteException {
        ...
        if (annotation == null) {
            ...
            /*
             * If the template file name (minus suffix) has no dots, make it relative to the binder's package, otherwise
             * slashify the dots
             */
    ...
    private Document readDocument(TreeLogger logger, ResourceOracle resourceOracle, JClassType requestedType)
            throws UnableToCompleteException {
        ...
        try {
            ...
            logger.log(Type.ERROR, "Error parsing XML (line " + e.getLineNumber() + "): " + e.getMessage(), e);
            throw new UnableToCompleteException();
        }

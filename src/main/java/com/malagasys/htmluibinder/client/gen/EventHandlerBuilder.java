...
public class EventHandlerBuilder implements PartBuilder {
  ...
  private void writeEventHandlerMethod(HtmlUiGeneratorContext context, String fieldName,
      JMethod targetMethod) throws UnableToCompleteException {
    TreeLogger logger = context.getTreeLogger();
    ...
    writeEventHandlerImplementation(context, field.getName(), targetMethod, methods[0], methods[1],
        eventType);
  }

  private void writeEventHandlerImplementation(HtmlUiGeneratorContext context, String fieldName,
      JMethod targetMethod, JMethod addHandlerMethod, JMethod handlerMethod, JClassType eventType)
    ...
    srcWriter.println("public void %s(%s event) {", handlerMethod.getName(), eventType
  ...
  private JMethod[] findEventListenerMethod(TreeLogger logger, JField field, JClassType eventType)
      throws UnableToCompleteException {
    TreeLogger debugLogger =
        logger.branch(Type.DEBUG, "Find event listener method for the field `" + field.getName()
        ...
        if (parameterTypes != null && parameterTypes.length == 1
          ...
          for (JMethod eventHandlerMethod : eventHandler.getMethods()) {
            ...
            if (types != null && types.length == 1 && types[0].equals(eventType)) {
              debugLogger.log(Type.DEBUG, "Found a method matching the eventType : `"
          ...
          if (matchingMethods.size() > 1) {
            logger.log(Type.ERROR, "Ambiguous. There are " + matchingMethods.size()
                + " methods in the event handler interface " + "`" + eventHandler.getName()
                + "' accepting a parameter of type `" + eventType.getName() + "'");
            throw new UnableToCompleteException();
          }
    ...
    if (methods == null) {
      logger.log(Type.ERROR, "There is no eventhandler to handle event of type `"
          + eventType.getName() + "'" + " that can be added to field `" + field.getName() + "("
          + field.getType() + ")'");
      throw new UnableToCompleteException();
    }
  ...
  private JClassType findEventType(TreeLogger logger, JMethod handlerMethod, JField ownerField)
    ...
    if (types == null || types.length == 0) {
      logger.log(Type.ERROR, "Ambiguous handler method : `" + handlerMethod
          + "'. Expected 1 parameter" + " but found " + ((types == null) ? 0 : types.length));
      throw new UnableToCompleteException();
    }

...
/**
 * For each field annotated with {@link HtmlUiField}, generate a method that create the field. At the end create a
 * method that assign each field to the builder target.
 * 
 * @author hermann
 */
...
class WidgetFieldsBuilder implements PartBuilder {
    ...
    public void generate(HtmlUiGeneratorContext ctx) throws UnableToCompleteException {
        ...
        for (JField field : containerType.getFields()) {
            ...
            if (htmlFieldAnnotation != null) {
                if (field.isPrivate()) {
                    ctx.getTreeLogger().log(Type.ERROR,
                            "Field `" + field.getName() + "' is annotated " + "with @HtmlUiField but is private.");
    ...
    private String writeMethodToInitField(JField field, HtmlUiField annotatedWith, JClassType containerType,
        ...
        if (context.getId(htmlUiId) != null) {
            ...
            srcWriter.println("transferNodesAttributes(htmlPanel.getElementById(\"%s\"), container.%s.getElement());",
                    id, field.getName());
            ...
            context.getTreeLogger().log(Type.ERROR,
                    "There is no `htmlui:id' tag with the value `" + htmlUiId + "' found in the html template file.");
            throw new UnableToCompleteException();
        }
